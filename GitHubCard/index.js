...
axios
	...
	.then((response) => {
		...
		axios
			...
			.then((response) => {
				response.data.forEach((user) => {
					...
					axios
						...
						.then((response) => {
							//Handle success - pass response date into created Card function and add this to .cards class var
							cards.appendChild(createCard(response));
						})
						//Handle error
			...
			.catch((error) => console.log('OH NO: ', error)); //Do we need a catch for each axios get request?
	})
	.catch((error) => console.log('OH NO: ', error)); //Do we need a catch for each axios get request?
...
function createCard(user) {
	...
	location.textContent = `Location on Planet Earth: ${user.data.location}`;
	...
	followers.textContent = `Loyal Followers: ${user.data.followers}`;
	//add followers p var to info div
	info.appendChild(followers);

	//add text content to following p var
	following.textContent = `Following: ${user.data.following} other geeks.`;
